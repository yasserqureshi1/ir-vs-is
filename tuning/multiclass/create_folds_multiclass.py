...
data_path = '/home/eng/esrdfn/tuning/multiclass/final-data/'
results_path = '/home/eng/esrdfn/tuning/multiclass/results/'


def feature_selection(data0, data1):
    _, p_val = mannwhitneyu(data0, data1)
    rej, _, _, _ = multipletests(p_val, alpha=0.05, method='holm')
    columns = data0.columns[rej]
    return list(columns)


def run_feature_selection(df_hyp, df_hyp_target):
    ...
    columns = np.setdiff1d(columns, to_drop)
    return columns
...
def run(window_size, overlap, train_trials, index):
    ...
    columns = run_feature_selection(x_train, y_train)
...
if __name__ == '__main__':
    ...
    #joblib.dump(jobs, results_path + f'results_multiclass.dat')