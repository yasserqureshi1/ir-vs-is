...
data_path = '/home/eng/esrdfn/tuning/ir-vs-is/final-data/'
results_path = '/home/eng/esrdfn/tuning/ir-vs-is/results/'

threshold_type = 'mutual_info' # 'gradient'

folds = [
    [1,14,4,9], [1,14,5,10], [0,14,5,9], [0,13,4,9], [0,14,4,9]
]
...
def run(window_size, overlap, train_trials, index):
    ...
    print('dump files')
    joblib.dump(x_train, data_path + f'xtrain_{window_size}_{overlap}_fold{index}_{threshold_type}.dat')
    joblib.dump(y_train, data_path + f'ytrain_{window_size}_{overlap}_fold{index}_{threshold_type}.dat')
    joblib.dump(train_os, data_path + f'xtrainos_{window_size}_{overlap}_fold{index}_{threshold_type}.dat')
    joblib.dump(train_targets_os, data_path + f'ytrainos_{window_size}_{overlap}_fold{index}_{threshold_type}.dat')
    joblib.dump(x_test, data_path + f'xtest_{window_size}_{overlap}_fold{index}_{threshold_type}.dat')
    joblib.dump(y_test, data_path + f'ytest_{window_size}_{overlap}_fold{index}_{threshold_type}.dat')
...
if __name__ == '__main__':
    ...
    #jobs = pd.DataFrame(jobs_format)
    #joblib.dump(jobs, results_path + f'results_{threshold_type}.dat')
