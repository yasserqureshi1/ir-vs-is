...
file = f'logistic-regression/logistic_results_{job_number}.csv'
...
def run_model(model_type, metadata, param, q):
    ...
    if model_type == 'random forests':
        ...
        try:        
            ...
            result_string = f'{str(fold_id)},{window_size},{overlap},{penalty},{c},{solver},{max_iter},{test_perf["balanced accuracy"]},{test_perf["roc auc"]},{train_perf["balanced accuracy"]},{train_perf["roc auc"]}\n'
            q.put(result_string)
        
            print(f'{str(fold_id)},{window_size},{overlap},{penalty},{c},{solver},{max_iter},{test_perf["balanced accuracy"]},{test_perf["roc auc"]},{train_perf["balanced accuracy"]},{train_perf["roc auc"]}')
        except:
            pass
...
def list_of_params(model_type, params):
    ...
    if model_type == 'logistic regression':
        for (penalty, c, solver, max_iter) in itertools.product(*params):
            param = {
                'penalty': penalty, 
                'C': c,
                'solver': solver,
...
if __name__ == '__main__':
    ...
    if model_type == 'random forests':
        ...
        try:
            ...
            RESULTS = open(file, 'w+')
            RESULTS.write('train trials,window size,window overlap,penalty,c,solver,max iter,balanced accuracy,roc auc score,train balanced accuracy,train roc auc score\n')
    else:
        raise ValueError('`model_type` does not exist')
    ...
    # ---------------------- CV FOLDS ----------------------
    if model_type == 'logistic regression':
        all_params = list_of_params(model_type, logistic_params)
    else:
        raise ValueError('`model_type` does not exist')
