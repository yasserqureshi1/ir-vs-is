...
file = f'xgboost/xgboost_results_{job_number}.csv'
...
if __name__ == '__main__':
    ...
    if model_type == 'random forests':
        ...
        try:
            ...
            RESULTS = open(file, 'w+')
            RESULTS.write('train trials,window size,window overlap,learning rate,n estimators,max depth,subsample,colsample bytree,reg alpha,reg lambda,min child weight,balanced accuracy,roc auc score,train balanced accuracy,train roc auc score\n')
    else:
        raise ValueError('`model_type` does not exist')
